...
int AvH264_Decoder::avframe2cvmat(AVFrame* frame, cv::Mat& res)
{
...
AVFrame* AvH264_Decoder::cvmat2avframe(const cv::Mat& mat)
{
...
AvH264_Decoder::~AvH264_Decoder()
{
    AvH264_Decoder::close();
}

int AvH264_Decoder::init()
{
...
int AvH264_Decoder::decode(uint8_t* pDataIn, int nInSize, cv::Mat& res)
{
    packet.size = nInSize;
    packet.data = pDataIn;
    int ret;
